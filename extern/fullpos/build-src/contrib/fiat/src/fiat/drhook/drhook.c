...
void c_drhook_getenv_(const char *s, char *value, int slen, const int valuelen)
{
  ...
  if (!value || valuelen <= 0) return;
  memset(value, ' ', (size_t)valuelen);
  if (!s || slen <= 0) return;

  n = slen < (int)sizeof(name) - 1 ? slen : (int)sizeof(name) - 1;
  while (n > 0 && s[n - 1] == ' ') --n;
  ...
  env = getenv(name);
  if (env) {
    int m = (int)strlen(env);
    if (m > valuelen) m = valuelen;
    memcpy(value, env, (size_t)m);
  }
}
...
void c_drhook_start_(const char *name, const int *thread_id, double *key,
                     const char *filename, const int *sizeinfo,
...
void c_drhook_end_(const char *name, const int *thread_id, const double *key,
                   const char *filename, const int *sizeinfo,
                   int name_len, int filename_len)
{
  (void)name;
...
void c_drhook_watch_(const int *onoff, const char *array_name, const void *array_ptr,
                     const int *nbytes, const int *abort_if_changed,
                     const int *printkey, const int *nvals,
                     const int *print_traceback_when_set, int array_name_len)
{
  (void)onoff;
  (void)array_name;
  (void)array_ptr;
  (void)nbytes;
  (void)abort_if_changed;
  (void)printkey;
  (void)nvals;
  (void)print_traceback_when_set;
  (void)array_name_len;
}
...
void c_drhook_prof_(void) {}

void trapfpe_slave_threads_(const int *silent)
{
  (void)silent;
}

void trapfpe_slave_threads(const int *silent)
{
  trapfpe_slave_threads_(silent);
}

double util_walltime_(void)
{
...
int drhook_active(void)
{
  return drhook_lhook;
}

void Dr_Hook(const char *name, int option, double *handle, const char *filename,
             int sizeinfo, int name_len, int filename_len)
{
  (void)name;
  (void)option;
  (void)filename;
...
{
  if (drhook_abort_funptr) drhook_abort_funptr(file, line, txt);
  abort();
}

void drhook_calltree(void) {}
